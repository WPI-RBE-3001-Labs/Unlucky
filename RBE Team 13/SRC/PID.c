...
signed int calcPID(char link, int setPoint, int actPos)
{
	...
	if (link == 'L')
	{
		pidValue = PrevL + PID.Kp_L * ((1 + TiL + TdL) * error  + (-1 - 2 * TdL) * LastErrorL + TdL * PrevLastErrorL);
		PrevL = pidValue;
	...
	{
		pidValue = PrevH + PID.Kp_H * ((1 + TiH + TdH) * error  + (-1 - 2 * TdH) * LastErrorH + TdH * PrevLastErrorH);
		PrevH = pidValue;
